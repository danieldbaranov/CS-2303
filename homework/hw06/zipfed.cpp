...
/** Alternate ctor for Zipfed.
 *  Initializes members into their final values immediately
 */

Zipfed::Zipfed(char * zc, char * zt, char * c, char * s, double la, double lo){
  zipcode = strdup(zc);
  zctype = INVALID;
  city = strdup(c);
  state = strdup(s);
...
/** Function to print zipcode structure to a file
 *
 * @param file is the file to be printed to. In this case csv
 */
...
/** Function to get the state that the zip code belongs to
 */
std::string Zipfed::getstate(){
  return state;
}

/** Function to get the city that the zip code belongs to
 */
